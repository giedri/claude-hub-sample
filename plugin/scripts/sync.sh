...
read_config() {
    ...
    REPO_URL="$(cat "$REPO_URL_FILE" 2>/dev/null | tr -d '[:space:]')"
    if [ -z "$REPO_URL" ]; then
...
check_ttl() {
    ...
    last_sync_time=$(cat "$LAST_SYNC_FILE" 2>/dev/null | head -1)
    local now
    now=$(date +%s)
...
# ---------------------------------------------------------------------------
# Merge MCP server configs from org and team settings.json into user settings
#
# Reads mcpServers from org/settings.json and teams/<team>/settings.json,
# merges them (team overrides org for same server name), and writes the
# result into ~/.claude/settings.json without touching other keys.
#
# Tracks which servers were added by hub in a separate file so stale
# entries can be cleaned up when servers are removed from the repo.
# ---------------------------------------------------------------------------
...
sync_mcp_servers() {
    ...
    if [ "$has_org" -eq 0 ] && [ "$has_team" -eq 0 ]; then
        # No MCP configs to sync, but clean up any previously managed servers
        if [ -f "$MANAGED_MCP_FILE" ]; then
            _remove_managed_mcp_servers
            rm -f "$MANAGED_MCP_FILE"
            log "Removed previously managed MCP servers (none configured now)"
        fi
        return 0
    fi

    # Merge org (if not MDM) and team MCP servers, then write to user settings
    python3 - "$org_settings" "$team_settings" "$USER_SETTINGS" "$MANAGED_MCP_FILE" "$mdm_mode" <<'PYEOF'
import json, sys, os

org_path, team_path, settings_path, managed_path, mdm_mode = sys.argv[1:6]
mdm_mode = mdm_mode == "1"

# Collect hub-managed MCP servers (org + team, team wins on conflict)
hub_servers = {}

if not mdm_mode and os.path.isfile(org_path):
    try:
...
# Read previously managed server names for cleanup
prev_managed = set()
if os.path.isfile(managed_path):
    with open(managed_path) as f:
        prev_managed = set(line.strip() for line in f if line.strip())
...
# Remove previously hub-managed servers that are no longer in the repo
for name in prev_managed:
    if name not in hub_servers and name in mcp:
        del mcp[name]

# Add/update hub-managed servers
mcp.update(hub_servers)
...
# Write current managed server names for next sync
with open(managed_path, "w") as f:
    for name in sorted(hub_servers.keys()):
        f.write(name + "\n")
PYEOF

    if [ $? -eq 0 ]; then
        local count
        count=$(wc -l < "$MANAGED_MCP_FILE" 2>/dev/null | tr -d ' ')
        log "Synced ${count:-0} MCP server(s) to $USER_SETTINGS"
    else
        log "WARN: MCP server sync failed"
    fi
}

# Remove hub-managed MCP servers from user settings (cleanup helper)
_remove_managed_mcp_servers() {
    [ -f "$USER_SETTINGS" ] || return 0
    [ -f "$MANAGED_MCP_FILE" ] || return 0
    command -v python3 >/dev/null 2>&1 || return 0

    python3 - "$USER_SETTINGS" "$MANAGED_MCP_FILE" <<'PYEOF'
import json, sys, os
settings_path, managed_path = sys.argv[1:3]
