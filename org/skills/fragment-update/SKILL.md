...
3. Locate the latest fragment source. Check `~/.claude/claude-hub/repo/fragments/<fragment>/` first, then fall back to `~/.claude/claude-hub/repo/examples/fragments/<fragment>/`. If neither exists, inform the developer that the fragment no longer exists in the central repo. Stop.
